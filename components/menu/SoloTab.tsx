...
const SoloTab: React.FC = () => {
  const { difficulty, setDifficulty, startGame, matchDuration, setMatchDuration, gameMode, setGameMode, t } = useGameStateContext();
  const [isModeSelected, setIsModeSelected] = useState(false);

  if (!isModeSelected) {
    return (
      <div className="w-full max-w-xl mx-auto space-y-4 md:space-y-4.5 animate-scaleUp">
        <h3 className="text-xs md:text-sm font-bold uppercase tracking-widest text-cyan-400">{t('solo.challengeAi')}</h3>
        
        <div className="space-y-2">
          <h4 className="text-[9.5px] font-black uppercase tracking-widest text-zinc-450 text-left">{t('solo.matchPhysics')}</h4>
          <div className="grid grid-cols-2 gap-2.5">
            {[
              { id: 'standard', label: t('solo.standardMode'), desc: t('solo.standardModeDesc'), icon: Zap },
              { id: 'manual', label: t('solo.manualMode'), desc: t('solo.manualModeDesc'), icon: Sliders }
            ].map((modeOpt) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[8px] font-medium text-zinc-550 leading-none">{modeOpt.desc}</span>
                </button>
              );
            })}
          </div>
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-4 md:space-y-4.5 animate-scaleUp">
      <div className="flex items-center justify-between">
        ...
        >
          <span>&larr;</span> {t('solo.back')}
        </button>
      </div>

      <div className="space-y-2">
        <h4 className="text-[9.5px] font-black uppercase tracking-widest text-zinc-450 text-left">{t('solo.aiDifficulty')}</h4>
        <div className="grid grid-cols-3 gap-2.5">
          {[
            { id: Difficulty.EASY, label: t('solo.easy'), desc: t('solo.easyDesc'), color: 'border-emerald-500/40 text-emerald-400 bg-emerald-950/10' },
            { id: Difficulty.MEDIUM, label: t('solo.medium'), desc: t('solo.mediumDesc'), color: 'border-amber-500/40 text-amber-400 bg-amber-950/10' },
            { id: Difficulty.HARD, label: t('solo.hard'), desc: t('solo.hardDesc'), color: 'border-rose-500/40 text-rose-400 bg-rose-950/10' }
          ].map((diff) => (
            <button
              key={diff.id}
            ...
            >
              ...
              <span className="text-[8px] font-medium text-zinc-550 leading-none">{diff.desc}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <h4 className="text-[9.5px] font-black uppercase tracking-widest text-zinc-450 text-left">{t('solo.matchDuration')}</h4>
        <div className="grid grid-cols-3 gap-2.5">
          {[
            { id: 60, label: t('solo.oneMinute'), desc: t('solo.lightningMatch') },
            { id: 180, label: t('solo.threeMinutes'), desc: t('solo.classicTime') },
            { id: 300, label: t('solo.fiveMinutes'), desc: t('solo.tacticalWar') }
          ].map((timeOpt) => (
            <button
              key={timeOpt.id}
            ...
            >
              ...
              <span className="text-[8px] font-medium text-zinc-550 leading-none">{timeOpt.desc}</span>
            </button>
          ))}
        </div>
      </div>
